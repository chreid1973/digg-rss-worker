    ...
    edges {
      ...
      for (const windowMs of windowsMs) {
        ...
        for (const where of whereVariants) {
          // Try the richer query (with account.username). If Digg rejects that field,
          // retry once with the base query so feeds never go down.
          const queries = [gqlQueryWithAccount, gqlQueryBase];
          ...
          for (const q of queries) {
            ...
            if (resp.ok && json?.data?.posts?.edges && !json?.errors) {
              got = json.data.posts.edges;
              break;
            } else {
              err = json?.errors || json || { status: resp.status };
            }
          }
